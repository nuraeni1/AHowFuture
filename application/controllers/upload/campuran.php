...
class campuran extends CI_Controller {
	...
	function index()
	{
		$data['error']= '';
		$data['message']='';
		$this->load->view('upload/v_upload_campuran',$data); 
                //untuk memanggil tampilan yang terdapat di folder view bernama upload_form.php
		$this->content();
	}
	function do_upload()
	{
        $config['upload_path'] = './CAMPURAN/'; //nama folder untuk menyimpan file
	...
	$this->load->view('upload/v_download_campuran', $data);	//tampilan untuk mendownload file
	}
	function download() 
    {	 
